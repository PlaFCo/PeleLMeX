...
PeleLM::ionDriftVelocity(std::unique_ptr<AdvanceAdvData>& advData)
{
  ...
  if (m_ef_model == EFModel::EFglobal) { // E is grad PhiV
    ...
    //        auto const& rhoYOld =  ldataOld_p->state.const_array(mfi,
    //        FIRSTSPEC); auto const& rhoYNew =
    //        ldataNew_p->state.const_array(mfi, FIRSTSPEC); auto const& ChO =
    //        ChargeOld_CC.array(mfi); auto const& ChN =
    //        ChargeNew_CC.array(mfi); amrex::ParallelFor(
    ...
    //            amrex::ParallelFor(
    //              bx,
    //              [ef_old, ef_new, ChO, ChN, dx, factor] AMREX_GPU_DEVICE(int
    //              i, int j, int k) noexcept {
    //                Real chrg_p = 0.25 * ( ChO(i-1, j, k) + 2 * ChO(i, j, k) +
    //                ChO(i+1, j, k)); Real chrg_m = 0.25 * ( ChO(i, j, k) + 2*
    //                ChO(i-1, j, k) + ChO(i-2, j, k)); ef_old(i, j, k) =
    //                (chrg_p - chrg_m)*factor;///dx[0]; chrg_p = 0.25 * (
    //                ChN(i-1, j, k) + 2 * ChN(i, j, k) + ChN(i+1, j, k));
    //                chrg_m = 0.25 * ( ChN(i, j, k) + 2* ChN(i-1, j, k) +
    //                ChN(i-2, j, k)); ef_new(i, j, k) = (chrg_p -
    //                chrg_m)*factor;///dx[0];
    //              });
    //          } else if (idim == 1) {
    //            amrex::ParallelFor(
    ...
    //              [ef_old, ef_new, ChO, ChN, dx, factor] AMREX_GPU_DEVICE(int
    //              i, int j, int k) noexcept {
    //                Real chrg_p = 0.25 * ( ChO(i, j-1, k) + 2 * ChO(i, j, k) +
    //                ChO(i, j+1, k)); Real chrg_m = 0.25 * ( ChO(i, j, k) + 2 *
    //                ChO(i, j-1, k) + ChO(i, j-2, k)); ef_old(i, j, k) =
    //                (chrg_p - chrg_m)*factor;///dx[1]; chrg_p = 0.25 * (
    //                ChN(i, j-1, k) + 2 * ChN(i, j, k) + ChN(i, j+1, k));
    //                chrg_m = 0.25 * ( ChN(i, j, k) + 2 * ChN(i, j-1, k) +
    //                ChN(i, j-2, k)); ef_new(i, j, k) = (chrg_p -
    //                chrg_m)*factor;///dx[1];
    //              });
    //          } else if (idim == 2) {
    //            amrex::ParallelFor(
    ...
    //              [ef_old, ef_new, ChO, ChN, dx, factor] AMREX_GPU_DEVICE(int
    //              i, int j, int k) noexcept {
    //                ef_old(i, j, k) = (ChO(i, j, k) - ChO(i, j,
    //                k-1))*factor;///dx[2]; ef_new(i, j, k) = (ChN(i, j, k) -
    //                ChN(i, j, k-1))*factor;///dx[2];
    //              });
    //          }
    //        }
    //      }
